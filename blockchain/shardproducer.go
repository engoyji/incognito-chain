	...
	for _, beaconBlock := range beaconBlocks {
		...
		for _, l := range beaconBlock.Body.Instructions {
			...
			case metadata.PDECrossPoolTradeRequestMeta:
				if len(l) >= 4 {
					newTx, err = blockGenerator.buildPDECrossPoolTradeIssuanceTx(l[2], l[3], producerPrivateKey, shardID)
				}
			...
			case metadata.PDEWithdrawalWithPRVFeeRequestMeta:
				if len(l) >= 4 {
					newTx, err = blockGenerator.buildPDEWithdrawalWithPRVFeeTx(l[2], l[3], producerPrivateKey, shardID)
				}
