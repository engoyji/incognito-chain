...
func (pc PDECrossPoolTradeRequest) ValidateTxWithBlockChain(txr Transaction, bcr BlockchainRetriever, shardID byte, db *statedb.StateDB) (bool, error) {
	// NOTE: verify supported tokens pair as needed
	return true, nil
}

func (pc PDECrossPoolTradeRequest) ValidateSanityData(bcr BlockchainRetriever, txr Transaction, beaconHeight uint64) (bool, bool, error) {
	...
	if txr.GetType() == common.TxCustomTokenPrivacyType {
		...
		if !txr.IsFullBurning(bcr, beaconHeight) {
			return false, false, errors.New("Must send coins to burning address")
		}
		prvAmt, pTokenAmt := txr.GetFullTxValues()
...
func (pc *PDECrossPoolTradeRequest) BuildReqActions(tx Transaction, bcr BlockchainRetriever, shardID byte) ([][]string, error) {
